...
class CrosstalkManager():
    ...
    def run_crosstalk_task(self, sensor_id, sensor_id2):
        ...
        for xpos, ypos in self.xtalk_dict[sensor_id]:
            xtalk_files = glob.glob(os.path.join(self.main_dir, 'xtalk_{0}_{1}*'.format(xpos, ypos),
                                                 '*_{0}.fits'.format(sensor_id)))[0]
#            infile = os.path.join(self.output_dir, sensor_id,
...
#            print('made file {0}'.format(infile))

            xtalk_files2 = glob.glob(os.path.join(self.main_dir, 'xtalk_{0}_{1}*'.format(xpos, ypos),
                                                  '*_{0}.fits'.format(sensor_id2)))[0]
#            infile2 = os.path.join(self.output_dir, sensor_id2,
#                                   '{0}_{1}_{2}_median.fits'.format(sensor_id2, xpos, ypos))
#            imutils.fits_median_file(xtalk_files2, infile2, bitpix=-32)
            infiles2.append(xtalk_files2)
#            print('made file {0}'.format(infile2))
...
def BOT(main_dir, output_dir='./'):
    ...
    for sensor_id, sensor_id2 in sensor_pairs:
        ...
        results.append(pool.apply_async(xtalk_manager.run_crosstalk_task, args = (sensor_id, sensor_id2)))
